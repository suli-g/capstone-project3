...
def find_user(username: str) -> User|None:
    ...
    with open(user_txt, 'r') as users:
        for line in users:
            credentials = tuple(line.strip().split(', '))
...
def add_task(
    ...
    completed: str = "No"):
        ...
        task_entry: str = entry_template.format(assign_to = assign_to, task = task, description = description, date_created = date_created, due_date = due_date, completed = completed)
...
def generate_reports():
    ...
    with open(task_txt, "r") as tasks, open(user_txt, "r") as users:
        ...
        for user_line in users:
            ...
            user_assigned = 0
            user_incomplete = 0
            user_percentage_of_total = 0.0
            user_percentage_completed = 0.0
            user_percentage_incomplete = 0.0
            user_percentage_overdue = 0.0
        ...
        # Write all the data to user_overview.txt and task_overview.txt
...
def edit_task(
        id: int, assigned_to: str|None = None, 
        due_date: str = "", completed: bool|None = None) -> str:
        ...
        for task_index, line in enumerate(tasks):
            if task_index == id:
                ...
                if task_completed == "No":
                    ...
                    result = "Task details edited succesfully."
                else:
                    lines.append(line)
